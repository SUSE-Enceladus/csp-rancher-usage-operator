...
func readConstantsFromEnv() error {
	var noBillThresholdStr = os.Getenv(cspBillingNoBillThreshold)
	var managerIntervalStr = os.Getenv(cspBillingManagerInterval)
...
func (m *UsageOperator) checkAndUpdateUserNotifications() error {
	...
	// in the errors field?
	var messagePrefix = "One or more errors detected by CSP billing. This Rancher instance may not be supported till the problem is resolved. Please resolve the following errors or contact SUSE for further assistance. "
	var cspErrors = ""
	...
	if len(cspErrors) > 0 {
		...
		cspErrors = messagePrefix + cspErrors
...
func (m *UsageOperator) getNodeCount() (int, error) {
